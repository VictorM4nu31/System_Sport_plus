<article class="cs-product-tile relative flex h-full flex-col overflow-hidden group">
    ...
    <div class="relative overflow-hidden bg-elevated">
        <a href="{{ route('usuario.productos.ver', $product->id) }}" class="block">
            @if($product->image)
        ...
        <form action="{{ route('usuario.deseos.agregar', $product->id) }}" method="POST" class="absolute right-3 top-14 md:opacity-0 md:transition md:duration-200 md:group-hover:opacity-100 focus-within:opacity-100">
            @csrf
    ...
    <div class="flex flex-1 flex-col gap-3 p-5">
        ...
        <h3 class="line-clamp-2 min-h-[3.5rem] text-heading-sm font-bold text-ink">
            <a href="{{ route('usuario.productos.ver', $product->id) }}" class="hover:underline">{{ $product->name }}</a>
        </h3>
        ...
        <div class="mt-auto flex flex-col gap-2 pt-2">
            @if($product->stock > 0)
                <form action="{{ route('usuario.carrito.agregar', $product->id) }}" method="POST">
                    @csrf
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('usuario.productos.ver', $product->id) }}" data-quickview="{{ $product->id }}" class="cs-button-secondary cs-focus flex-1 text-sm no-underline">Vista rápida</a>
