<x-app-layout>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white bg-opacity-95 shadow-lg rounded-lg p-4 sm:p-8">
            ...
            <form method="POST" action="{{ route('admin.categorias.update', $category->id) }}" class="flex flex-col gap-4">
                @csrf
                @method('PATCH')
                ...
                <div class="flex flex-col sm:flex-row justify-center gap-3 mt-6">
                    <a href="{{ route('admin.categorias.index') }}" class="inline-flex items-center justify-center px-4 py-2 min-h-[44px] rounded-md border border-line text-carbon hover:bg-paper transition w-full sm:w-auto">Cancelar</a>
                    <button type="submit"
