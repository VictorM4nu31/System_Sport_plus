    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white bg-opacity-95 shadow-xl rounded-lg p-8">
                ...
                <!-- Título centrado -->
                <h3 class="text-3xl font-bold text-center text-[#801336] mb-6">Gestión de Productos Deportivos</h3>

                <!-- Botón de agregar producto centrado -->
                <div class="flex justify-center mb-6">
                    <a href="{{ route('admin.products.create') }}"
                       class="px-6 py-3 bg-[#801336] text-white rounded-md font-semibold hover:bg-[#9b1a3e] transition duration-200 shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#801336]">
                        + Agregar Producto
                    </a>
                </div>
                ...
                <div class="bg-gray-50 p-6 rounded-lg mb-6">
                    <h4 class="text-lg font-semibold text-[#801336] mb-4">Filtros de Búsqueda</h4>
                    <form method="GET" action="{{ route('admin.products.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <!-- Búsqueda por nombre -->
                        <div>
                            <label class="block text-[#801336] font-medium mb-1">Nombre</label>
                        ...
                        <div class="flex items-end space-x-2">
                            <button type="submit" class="px-4 py-2 bg-[#801336] text-white rounded-md hover:bg-[#9b1a3e] focus:outline-none focus:ring-2 focus:ring-[#801336]">
                                Filtrar
                            </button>
                            <a href="{{ route('admin.products.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-400">
                                Limpiar
                            </a>
                        </div>
                    </form>
                        ...
                        <tbody>
                            @forelse ($products as $product)
                                <tr class="hover:bg-gray-50 transition duration-150 border-b">
                                    ...
                                    <td class="px-4 py-3">
                                        <div>
                                            <p class="font-semibold text-[#801336]">{{ $product->name }}</p>
                                            @if($product->sku)
                                    ...
                                    <td class="px-4 py-3">
                                        <div>
                                            ...
                                            @endif
                                            @if($product->model)
                                                <p class="text-sm text-gray-600">{{ $product->model }}</p>
                                            @endif
                                            @if($product->gender)
                                    ...
                                    <!-- Precio -->
                                    <td class="px-4 py-3">
                                        <span class="font-bold text-[#801336]">${{ number_format($product->price, 2) }}</span>
                                    </td>

                                    <!-- Stock -->
                                    <td class="px-4 py-3">
                                        <span class="font-semibold {{ $product->stock <= 0 ? 'text-red-600' : ($product->stock <= 10 ? 'text-yellow-600' : 'text-green-600') }}">
                                            {{ $product->stock }}
                                        </span>
                                        @if($product->stock <= 0)
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-col space-y-1">
                                            @if($product->sizes && count($product->sizes) > 0)
                                                <span class="text-xs text-gray-600">{{ count($product->sizes) }} tallas</span>
                                            @endif
                                            @if($product->colors && count($product->colors) > 0)
                                                <span class="text-xs text-gray-600">{{ count($product->colors) }} colores</span>
                                            @endif
                                            @if($product->isSyncedWithStripe())
                                                <span class="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">Stripe ✓</span>
                                            @else
                                                <span class="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">Sin Stripe</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <a href="{{ route('admin.products.show', $product->id) }}"
                                               class="text-blue-600 hover:text-blue-800 p-1 rounded" title="Ver detalles">
                                ...
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-gray-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm">Comienza agregando tu primer producto deportivo</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
